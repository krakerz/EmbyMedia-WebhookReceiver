...
<div class="px-4 sm:px-0">
    ...
    <div class="mb-6">
        <nav class="flex" aria-label="Breadcrumb">
            <ol class="flex items-center space-x-4">
                <li>
                    <a href="{{ route('webhooks.index') }}" class="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 flex items-center">
                ...
                <li>
                    <div class="flex items-center">
                        <svg class="flex-shrink-0 h-5 w-5 text-gray-300 dark:text-gray-700" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                            <path d="M5.555 17.776l8-16 .894.448-8 16-.894-.448z" />
                        </svg>
                        <span class="ml-4 text-sm font-medium text-gray-500 dark:text-gray-400">Media Details</span>
                    </div>
                </li>
            </ol>
        </nav>
    ...
    <!-- Media Header -->
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg dark:shadow-black/30 overflow-hidden mb-6">
        <div class="md:flex">
            <!-- Media Poster/Image -->
            <div class="md:w-1/3 lg:w-1/4">
                <div class="h-96 md:h-full bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-800">
                    @if(isset($webhook->metadata['poster_url']) || isset($webhook->metadata['backdrop_url']))
                        <img src="{{ $webhook->metadata['poster_url'] ?? $webhook->metadata['backdrop_url'] }}"
                             ...
                             onerror="handleMediaImageError(this, '{{ $webhook->uuid }}', '{{ $webhook->item_type }}', 'text-8xl')"
                ...
                <dl class="space-y-4">
                    ...
                    @if(config('app.debug') && $webhook->server_name)
                        <div class="flex justify-between">
                            ...
                            <dd class="text-sm text-gray-900 dark:text-gray-100">{{ $webhook->server_name }}</dd>
                        </div>
                    @endif
                    ...
                    @if($webhook->user_name)
                        <div class="flex justify-between">
                            ...
                            <dd class="text-sm text-gray-900 dark:text-gray-100">{{ $webhook->user_name }}</dd>
                        </div>
                    @endif

                    @if(isset($webhook->metadata['container']))
                        <div class="flex justify-between">
                            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Container</dt>
                            <dd class="text-sm text-gray-900 dark:text-gray-100">{{ strtoupper($webhook->metadata['container']) }}</dd>
                        </div>
                    @endif

                    @if(isset($webhook->metadata['width']) && $webhook->metadata['width'] > 0)
                        <div class="flex justify-between">
                            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Width</dt>
                            <dd class="text-sm text-gray-900 dark:text-gray-100">{{ $webhook->metadata['width'] }}px</dd>
                        </div>
                    @endif
                    ...
                    @if(isset($webhook->metadata['height']) && $webhook->metadata['height'] > 0)
                        <div class="flex justify-between">
                            ...
                            <dd class="text-sm text-gray-900 dark:text-gray-100">{{ $webhook->metadata['height'] }}px</dd>
                        </div>
                    @endif
                        ...
                        @foreach($webhook->metadata['external_urls'] as $externalUrl)
                            @if(isset($externalUrl['Name']) && isset($externalUrl['Url']))
                                <div class="flex justify-between items-center">
                                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ $externalUrl['Name'] }}</dt>
                                    <dd class="text-sm">
                ...
                <div class="p-6">
                    <dl class="space-y-4">
                        @foreach($webhook->metadata['provider_ids'] as $provider => $id)
                            @if($id)
                                <div class="flex justify-between items-center">
                                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ ucfirst($provider) }}</dt>
                                    <dd class="text-sm">
                                        <span class="text-gray-900 dark:text-gray-100 font-mono">{{ $id }}</span>
                                    </dd>
                                </div>
                            @endif
                        @endforeach
        ...
        <div class="bg-white dark:bg-gray-800 shadow-lg dark:shadow-black/30 rounded-xl overflow-hidden mb-6">
            ...
            <div class="p-6">
                <dl class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Event Type</dt>
                        <dd>
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 dark:bg-blue-900/40 text-blue-800 dark:text-blue-300">
                                {{ $webhook->event_type }}
                            </span>
                    ...
                    <div>
                        <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Received</dt>
                        <dd class="text-sm text-gray-900 dark:text-gray-100">
    ...
    @if($showRawData ?? true)
        ...
        <div class="bg-white dark:bg-gray-800 shadow-lg dark:shadow-black/30 rounded-xl overflow-hidden">
            ...
            <div id="raw-data-content" class="hidden p-6">
                <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">Complete webhook payload received from Emby server</p>
                <pre class="bg-gray-50 dark:bg-gray-900 text-gray-800 dark:text-gray-300 rounded-lg p-4 text-xs overflow-x-auto max-h-96 overflow-y-auto"><code>{{ json_encode($webhook->raw_payload, JSON_PRETTY_PRINT) }}</code></pre>
            </div>
        </div>
    @endif
</div>
